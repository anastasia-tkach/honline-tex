...
To achieve online \emph{joint} calibration and tracking, our algorithm presents two fundamental components which we refer to as \emph{intra-} and \emph{inter-frame} regression. The former gathers estimates from a single frame (intra, see \Section{independent}), while the latter integrates this knowlege across frames (inter, see \Section{combining}). 
\TODO{Describe the joint inter-intra frame regression}
...
Estimating uncertainty in the solution is essential for shape estimation. As we illustrate in \Figure{intra}, the confidence in regressed \emph{shape} parameters is conditional on the \emph{pose} of the current frame.
...
Its mean is the (final) solution of the (iterative) optimization problem in \Equation{energies}; its variance is computed according to \Equation{taylor-jacobian}, that is, from the Jacobian computed in the last iteration of~\Equation{energies}. \AT{we might have to also write the iterative version of \Equation{energies} as I did in Edoardo's paper to make this clear. Right now it sounds a bit... hand-wavy, ahahehehjaja}.

\input{algorithm/interframe}
\subsection{Inter-frame regression} 
% \subsection{Merging independent measurements}
\label{sec:combining}
Given the probabilistic interpretation in \Section{independent}, a \emph{joint} optimization over parameters can be obtained by cumulating the posterior distributions over the set of previous frames. This leads to the to the inductive update rules:
% 
\begin{align}
...
\end{align}
% 
In \Appendix{kalman}, we shown how \Equation{combining} is \emph{equivalent} to the Kalman filter update equations in \Algorithm{interframe}, with measurement $x_n^*$, and measurement noise covariance $\Sigma_n$. Such an equivalence is vital, as \cite{maybeck1979stochastic} noted how, under the \emph{same} assumptions we made in \Appendix{kalman}, a Kalman filter is optimal for our problem.
...
\AT{EDITED TILL HERE, HAVE TO BRING AMANDA TO THE HOSPITAL}

\input{algorithm/interframeplus}
