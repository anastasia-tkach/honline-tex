...
\AN{To derive our uncertainties, we start with converting the data terms of \Equation{energies} to probabilistic form}.
%To retrieve this information, we convert \AN{data terms of} \Equation{energies} to probabilistic form to derive our uncertainties. \AN{the sentence already starts with to retrieve this in}
%
...
\end{align}
% 
By temporarily omitting the frame index $n$ for conveniency, our optimization problem is rewritten in log-likelihood form:
%
\begin{align}
...
\end{align}
%
where $\Delta x=x - x^*$, and let {\small $\partial f(\star{x})/\partial x$} indicate the partial derivative of $f(x)$ evaluated at $\star{x}$  with an abuse of notation. We rewrite {\small $\bar{F}(x_n) = d_n - F(x_n)$} for brevity. Note how the Jacobian and the Hessian are respectively zero and positive definite at our optimal point $x^*$ 
:
%
\begin{align}
...
Its mean $x^*$ is the solution of the (iterative) optimization problem in \Equation{energies}; its covariance  $\star\Sigma_n$ is computed according to \Equation{taylor-hessian}, that is, from the 
...
\AN{Hessian of the objective function evaluated at the optimal solution $x^*$.}
...
\end{align}
% 
\AN{with $x^*_n$ the \lq\lq{}measurement\rq\rq{} of the parameters at the current frame $n$ and $\hat{x}$ the estimate of the parameters accumulated since the beginning of the sequence till the frame $n$.}
...
This optimization, which we refer to as \emph{split inter-frame} is arguably the simplest way of achieving an online parameter regression: by treating the results of the independent solve $\mathcal{N}(\star{x}_n, \star{\Sigma}_n)$ as the measurements in a Kalman filter.
...
like a finger floating in mid-air, or when the natural order of fingers $\{\text{index},\text{middle},\text{ring},\text{pinky}\}$ has been compromised. We overcome this problem by a set of \emph{linear} constraints that are conditionally enabled in the optimization when unfeasible configurations are detected (encoded via $\chi_c \in \{ 0,1 \}$):
% 
\begin{equation}
%E_\text{shape-valid} = \sum_{c=1}^C \chi_c  {\sum_{i=1}^{N_{c}} \kappa_{c_i} \beta_{c_i}}
E_\text{shape-valid} = \sum_{c=1}^C\| \chi_c \left< \beta, \kappa \right> \|_2^2
\label{eq:valideshape}
\end{equation}
% 
\AT{I have a question here, need examples of $\kappa$, also it'd be nice if we can write this in quadratic form, like the rest, rather than in linear form?} \AN{My mistake, they are already in quadratic form.
Example $\beta_{\text{index-base-x}} - \beta_{\text{index-base-radius}} - \beta_{\text{middle-base-x}} + \beta_{\text{middle-base-radius} > 0}$ prevents index and middle fingers from switching places}.
