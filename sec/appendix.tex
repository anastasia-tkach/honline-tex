...
We assume that while there is no new measurements, an estimate of the current hand parameters is given by the ones as the previous time-step up to Gaussian noise, that is, $x_n = x_{n-1} + w_{n-1}$. We also assume for now that depth sensor noise is normally distributed \AT{wouldn't sensor noise be accounted in \emph{measurement} noise?}, which actually is not the case. Thus, the states of our system can be approximated as following:
% 
\begin{align}
x_n &= x_{n - 1} + w_{n - 1} \\
z_n &= J x_n + v_n
