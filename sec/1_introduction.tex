...
Our core contribution is a principled way to integrate per-frame information into an online real-time pose/shape tracking algorithm: one that estimates the hand's \emph{pose}, while simultaneously refining the \emph{shape} of the user being tracked. That is, as more of the user's hand and articulation is observed during tracking, the more the tracking template is progressively adapted to match the performer, which in turns results in more accurate motion tracking. 
...
Our technique automatically estimates the \emph{confidence} in the per-frame parameter computations, and leverages this information to build a tracking model that selectively \emph{accumulates} confident parameter estimates over time. Assuming a reasonable performance by the user, our system typically constructs a fully calibrated model within 
\AN{a few seconds, while working as a real-time hand tracking system.}%\TODO{ten seconds}. 
The validity of our technical solution is corroborated by re-interpreting our algorithm as 
\AN{a Kalman filter, which finds a provably optimal estimate of the parameters given available online input}
%an optimal Kalman filter, %\AN{It sounds a bit like optimal is a type of kalman filter.}
