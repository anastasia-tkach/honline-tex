...
% \AN{It is not on Handy/Teaser, that sequence starts from fast motion where sensor data sets destroyed, which is bad for calibration.}.
% \AT{Just update the page with the new dataset!}
...
% 
Notice how the best performance is achieved by either (1) the intra-frame optimization, where per-frame \emph{overfitting} takes place, or (2) by offline calibration techniques, such as \OfflineSoft{} or \cite{taylor2016joint}, have a clear advantage over streaming algorithms (i.e. infinite memory).  \AN {I am confused about what are you trying to say with \lq\lq{}infinite memory\rq\rq{}}
